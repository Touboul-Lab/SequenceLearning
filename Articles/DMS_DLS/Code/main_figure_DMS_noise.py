...
noise_stim_list = [0., 0.001, 0.005, 0.01, 0.05, 0.1]
P_stim_by_pattern_list_noise = [[10, [3], [10, 15], [0.]]]
Apostpre_list = [-1.5, -1.0, -0.5, 0., 0.5, 1.0, 1.5, 2.]
Aprepost = -1.0
epsilon = 0.02
homeostasy = 0.95
noise_pattern = 0.2
...
c_list_noise = ['grey', 'yellow', 'orange', 'red', 'brown', 'black']

legend_noise = [Line2D([0], [0], color=colors[c_list_noise[k]], lw=1.,
                       label=r'$\lambda_{{\rm MSN}}{{=}}{}\,Hz$'.format(str(1000 * noise_stim_list[k])))
                for k in np.arange(len(noise_stim_list))]
...
for P, stim_by_pattern_list, n_pattern_list, stim_recall_list in P_stim_by_pattern_list_noise:
    for stim_by_pattern in stim_by_pattern_list:
        for n_pattern in n_pattern_list:
            gs_kw_2 = dict(hspace=0., wspace=0., height_ratios=[2., 4., 1., 1.],
                           width_ratios=[1.5, 15., 1.5],
                           top=1., bottom=0., left=0., right=1.)
            fig2, ax2_ = plt.subplots(figsize=(ratio * 18., ratio * 8.),
                                      ncols=3, nrows=4, gridspec_kw=gs_kw_2)
            gs_2 = ax2_[0, 0].get_gridspec()
            for ax_ in list(ax2_[:, 0]) + list(ax2_[:, 1]) + list(ax2_[:, 2]):
                ax_.remove()

            ax_2_title = fig2.add_subplot(gs_2[1, :])
            set_blank_axis(ax_2_title)

            ax_2_legend = fig2.add_subplot(gs_2[3, :])
            set_blank_axis(ax_2_legend)
            ax_2_legend.legend(handles=legend_noise, ncol=6, loc='center', frameon=True, edgecolor='grey',
                               facecolor='none', borderpad=1., fontsize=6)

            gs_kw_2_subplot_1 = dict(hspace=0., wspace=0., height_ratios=[0.5, 3.5])
            gs_2_subplot_1 = matplotlib.gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs_2[1, 1],
                                                                         **gs_kw_2_subplot_1)
            ax_2_iter = [fig2.add_subplot(gs_2_subplot_1[0, 0]), fig2.add_subplot(gs_2_subplot_1[1, 0])]

            legend_different_phases(ax_2_iter[0], params_simu, stim_recall_list[0])
            ax_2_iter[0].set_xticks([])
            ax_2_iter[0].set_yticks([])

            figure_different_phases(ax_2_iter[1], params_simu, ylim=params_simu['ylim_accuracy'])
            ax_2_iter[1].set_ylabel('Accuracy\nEnd of phase', labelpad=5)
            ax_2_iter[1].set_yticks([0.5, 1.])

            gs_kw_2_subplot_2 = dict(hspace=0., wspace=0., height_ratios=[0.5, 3.5],
            ...
            gs_2_subplot_2 = matplotlib.gridspec.GridSpecFromSubplotSpec(2, 4, subplot_spec=gs_2[1, 1],
                                                                         **gs_kw_2_subplot_2)
            ax_2_accuracy = [fig2.add_subplot(gs_2_subplot_2[1, 0]), fig2.add_subplot(gs_2_subplot_2[1, 1]),
                             fig2.add_subplot(gs_2_subplot_2[1, 2]), fig2.add_subplot(gs_2_subplot_2[1, 3])]

            for current_ax_number, current_ax in enumerate(ax_2_accuracy):
                current_ax.set_ylim(*params_simu['ylim_accuracy'])
                current_ax.set_yticks([])
                current_ax.set_xlabel('$A_{post-pre}$')

            for current_p, noise_stim in enumerate(noise_stim_list):
                value_analysis = {
                ...
                for current_l, stim_recall in enumerate(stim_recall_list):
                    ...
                    for current_ax_number, (current_ax_1, current_ax_2) in enumerate(zip(ax_1_accuracy, ax_2_accuracy)):
                        ...
                        figure_stats(current_ax_2, np.array(Apostpre_list),
                                     value_analysis['value_list_accuracy'][current_ax_number, :, current_l],
                                     color=colors[c_list_noise[current_p]], ref=params_simu['ref'])
            ...
            ax_2_title.set_title(
                ...
                '$ stimulations by pattern and $N_p=' + str(n_pattern) + '$ number of patterns', pad=20)
