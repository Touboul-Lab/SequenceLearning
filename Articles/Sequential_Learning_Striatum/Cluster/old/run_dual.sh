...
PARAMS_TXT="$SCRIPT_DIR/numeric_networks/Articles/Sequential_Learning_Striatum/Cluster/dual/params_pattern_dual_$NAME_PROJECT.txt"
...
if [ $HOSTNAME == 'cleps.CLEPS.eth.cluster' ]
then
    sbatch  --chdir=$DIRECTORY_PROJECT/$NAME_PROJECT --job-name=$NAME_PROJECT --array 1-$NUM_LINES_ARRAY /home/gvignoud/numeric_networks/Articles/Sequential_Learning_Striatum/Cluster/dual/Pattern_Dual.batch
elif [ $HOSTNAME == 'rioc' ]
then
    oarsub -S /home/rioc/gvignoud/numeric_networks/Articles/Sequential_Learning_Striatum/Cluster/dual/Pattern_Dual.batch --directory $DIRECTORY_PROJECT/$NAME_PROJECT --project $NAME_PROJECT --array $NUM_LINES_ARRAY
fi