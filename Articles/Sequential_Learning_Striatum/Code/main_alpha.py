...
from FiguresFunctions.OutputPatternClass import outputPatternClass, outputPatternExamples
from Articles.Sequential_Learning_Striatum.Code.InitParams import paramsGestPattern
...
PARAMS_GEST = paramsGestPattern()
PARAMS_GEST.init_main_pattern()

print(PARAMS_GEST.args.pattern)

if PARAMS_GEST.args.pattern == 'example':
    OUTPUT = outputPatternExamples(PARAMS_GEST.args, PARAMS_GEST.params_simu, PARAMS_GEST.params_pattern,
                                PARAMS_GEST.params_network)
else:
    OUTPUT = outputPatternClass(PARAMS_GEST.args, PARAMS_GEST.params_simu, PARAMS_GEST.params_pattern,
                                PARAMS_GEST.params_network)

last_time = time.time()
# print(PARAMS_GEST.params_network['alpha'])
PARAMS_GEST.params_network['taus']=0.1
print('tau_s=',PARAMS_GEST.params_network['taus'])
print('P=',PARAMS_GEST.params_network['P'])
for n_pattern in PARAMS_GEST.params_simu['n_pattern_list']:
    ...
    if not(OUTPUT.rerun):
        for num_simu in range(PARAMS_GEST.params_simu['num_simu']):
            ...
            SIMULATOR = PARAMS_GEST.simulator(params_simu=PARAMS_GEST.params_simu, params_pattern=PARAMS_GEST.params_pattern, params_network=PARAMS_GEST.params_network)
