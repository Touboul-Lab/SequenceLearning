...
name_J = ['No Inhibition','1<->2','1->2', 'Random']
...
for stop_learning in stop_learning_list:
    for P in P_list:
        for stim_by_pattern in stim_by_pattern_list:
            ...
            value_analysis = {
                'value_list_accuracy' : np.zeros((len(J_list), len(n_pattern_list)))
            }
            for l, n_pattern in enumerate(n_pattern_list) :
                ...
                figure_stats(ax1[1],None,value_analysis['value_list_accuracy'][:,l],color=colors['red'],xlabel='Type of STDP',ylabel='Accuracy',label=J_list)
            ...
            ax2.bar(n_pattern_list - 3*P/20., value_analysis['value_list_accuracy'][0], color=colors[c_list[0]], width=P/10., label=name_J[0])
            ax2.bar(n_pattern_list - P/20., value_analysis['value_list_accuracy'][1], color=colors[c_list[1]], width=P/10., label=name_J[1])
            ax2.bar(n_pattern_list + P/20., value_analysis['value_list_accuracy'][2], color=colors[c_list[2]], width=P/10., label=name_J[2])
            ax2.bar(n_pattern_list + 3*P/20., value_analysis['value_list_accuracy'][3], color=colors[c_list[3]], width=P/10., label=name_J[3])
